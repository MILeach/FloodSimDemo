...
//init and step functions added for communication

//init variables for communication
__FLAME_GPU_INIT_FUNC__ void initCommunication()
{
	int xmachine_agent_SoA_size = sizeof(xmachine_memory_agent_list);
	h_agents_pipe = (xmachine_memory_agent_list*)malloc(xmachine_agent_SoA_size);
...
//send data to Unreal
__FLAME_GPU_STEP_FUNC__ void communication()
{
	//copy options info
	CopyMemory(&options, optionsBuf, sizeof(struct options_data));
	if (options.quit == 1)
	{
		options.quit = 0;
		CopyMemory((PVOID)optionsBuf, &options, sizeof(struct options_data));
		exit(0);
	}
	//copy data everyframe
	CopyMemory(&options, optionsBuf, sizeof(int));
	if (options.lock == 0)
	{
		//agent data
		xmachine_memory_agent_list* d_agents_default = get_device_agent_default_agents();
		pedestrians.total = get_agent_agent_default_count();
		cudaMemcpy(h_agents_pipe, d_agents_default, sizeof(xmachine_memory_agent_list), cudaMemcpyDeviceToHost);
		for (int i = 0; i < pedestrians.total; ++i)
		{
			pedestrians.gender[i] = h_agents_pipe->gender[i];
			pedestrians.x[i] = h_agents_pipe->x[i];
			pedestrians.y[i] = h_agents_pipe->y[i];
			pedestrians.z[i] = h_agents_pipe->height[i];
			pedestrians.velX[i] = h_agents_pipe->velx[i];
			pedestrians.velY[i] = h_agents_pipe->vely[i];
			pedestrians.speed[i] = h_agents_pipe->speed[i];
			pedestrians.body_height[i] = h_agents_pipe->body_height[i];
		}
		CopyMemory((PVOID)pedestrianBuf, &pedestrians, sizeof(struct pedestrian_data));

		//water data
		xmachine_memory_FloodCell_list* d_floodCells_default = get_device_FloodCell_Default_agents();
		floodCells.total = get_agent_FloodCell_Default_count();
		cudaMemcpy(h_floodCell_pipe, d_floodCells_default, sizeof(xmachine_memory_FloodCell_list), cudaMemcpyDeviceToHost);
		for (int i = 0; i < floodCells.total; ++i)
		{
			floodCells.x[i] = h_floodCell_pipe->x[i];
			floodCells.y[i] = h_floodCell_pipe->y[i];
